...
**Note:** Device orientation data relies on the [DeviceOrientationEvent API](https://developer.mozilla.org/en-US/docs/Web/API/DeviceOrientationEvent), which is only available on mobile devices and via secured connections (HTTPS). If the API is not available, the hook will return `null` for all values.
