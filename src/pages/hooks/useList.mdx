...
`useList` is a hook that provides a way to manage a [List](<https://en.wikipedia.org/wiki/List_(abstract_data_type)>) of items within your React components. See the [API Reference](#api-reference) for more details.
...
export default function MyListComponent() {
  ...
  const { items, head, size, prepend, remove, append } = useList<string>([
    'Item 1',
    'Item 2',
    'Item 3',
  ])
...
The `useList` hook returns several properties of the List along with several functions for interacting with it.
