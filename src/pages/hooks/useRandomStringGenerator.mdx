...
The `useRandomStringGenerator` hook provides a convenient way to generate randomized strings with customizable settings. This hook enables you to define string length and choose whether to include symbols, numbers, and uppercase letters. The string is updated automatically whenever the settings are changed.
...
2. Import and use the `useRandomStringGenerator` hook in your component:

```tsx
import { useRandomStringGenerator } from '@ridgehkr/usefulHooks'
```

## Example

The `useRandomStringGenerator` hook takes two parameters: the `value` that you want to throttle and an optional `interval` in milliseconds. It returns the throttled value, which is updated no more frequently than the specified `interval`.

Here's an example of how you can use the `useRandomStringGenerator` hook in your React component:
...
In this example, the `useRandomStringGenerator` hook is used to generate a random string based on the current settings. The resulting string is updated automatically whenever the settings are changed.

## Parameters

`useRandomStringGenerator()` accepts an object of arguments to initially define the properties of the generated string. All of these parameters can be changed later using the [functions returned](#return-value) by the hook.
...
const { value } = useRandomStringGenerator({
  length: 24,
  symbols: true,
  numbers: true,
  uppercase: true,
...
The `useRandomStringGenerator` hook returns an object with the following properties:
